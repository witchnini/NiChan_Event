...
import { Eye, EyeOff, UserPlus, MailCheck, Send } from "lucide-react";
...
const Register = () => {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      // Silently handle — the API always returns success
    } finally {
      setResending(false);
    }
  };
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="w-full max-w-md">
          ...
          {!registered ? (
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <h2 className="font-serif text-display-sm text-foreground mb-2">Kiểm tra email!</h2>
              <p className="font-body text-muted-foreground mb-6">
                Chúng tôi đã gửi email xác thực đến{" "}
                <span className="text-foreground font-semibold">{formData.email}</span>.
                Vui lòng kiểm tra hộp thư (kể cả mục Spam) và nhấn liên kết xác thực.
              </p>
