...
const AdminProfile = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await apiClient.get<AdminProfileResponse>("/admin/profile");
        if (cancelled) return;
        setProfile({
          name: data.adminProfile?.fullName || data.displayName || "",
          email: data.email || "",
          phone: data.phone || "",
          role: data.role,
          joinDate: formatDate(data.createdAt),
          address: data.adminProfile?.address || "",
          bio: data.adminProfile?.bio || "",
  ...
  const stats = useMemo(() => [
    ...
    { label: "Đăng nhập gần nhất", value: profile.lastLoginAt, icon: User },
  ], [profile]);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex flex-col sm:flex-row items-start gap-6">
          ...
          <div className="flex-1">
            ...
            <p className="font-body text-sm text-primary font-semibold mt-1">Quản trị viên</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            ...
            {editing ? (
              ...
              <p className="font-body text-sm text-foreground py-2">{profile.phone || "Chưa cập nhật"}</p>
            )}
          </div>
          <div>
            ...
            {editing ? (
              ...
              <p className="font-body text-sm text-foreground py-2">{profile.address || "Chưa cập nhật"}</p>
            )}
          </div>
