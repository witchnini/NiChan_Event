...
const AdminNotifications = () => {
  ...
  const markAllRead = async () => {
    await Promise.all(notifications.filter(n => !n.isRead).map(n => apiClient.patch(`/admin/notifications/${n.id}/read`)));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-serif text-headline-lg text-foreground">Quan ly thong bao</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Tim kiem thong bao..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9" />
        </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          ...
          filtered.map((n, i) => (
            <motion.div key={n.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className={`flex items-start gap-4 p-4 rounded-xl transition-all ${!n.isRead ? "bg-primary/5 shadow-ambient" : "bg-surface-lowest hover:bg-surface-low"}`}>
              <div className="mt-1 shrink-0">{!n.isRead ? <Mail size={18} className="text-primary" /> : <MailOpen size={18} className="text-muted-foreground" />}</div>
              ...
              {!n.isRead && (
                <button onClick={() => markAsRead(n.id)} className="p-1.5 rounded-lg hover:bg-surface-low text-muted-foreground hover:text-foreground transition-colors" title="Danh dau da doc">
                  <Check size={14} />
                </button>
              )}
            </motion.div>
